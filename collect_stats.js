...
const collect_stats = (() => {
	...
	const config = require(process.env.CONFIG_PATH__NFT).collect_stats ;
	...
	const fetch = (() => {
		 ...
		 */
		const request = async (slug) => new Promise(g =>
			locks.request("collect_stats:request", async () => {
				const res = {
					success: false,
					body: "init_error",
				...
				g(res);
				await asleep(
					rand(config.fetch.delay.min, config.fetch.delay.max)
				);
				return true;
			})
		);
		...
		redis_connect.request.on["collect_stats:fetch:many"] = async (slugs_array) => {
			...
			await Promise.all(
				[...slugs_to_check].map(async slug => {
					const _res = {success: false, body: "init_error"};
					try {
						_res.body = await fetch.main(slug);
						_res.success = true;
					} catch (e) {
						_res.success = false;
						_res.body = e;
